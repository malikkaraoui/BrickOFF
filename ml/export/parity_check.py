"""CH-3 jalon 3.2 (DRY-RUN) — Parité numérique PyTorch (référence) vs CoreML.

Protocole adapté du plan (04_CH3_EXPORT_MOBILE.md §3.2) : 50 images du test split
(échantillon déterministe seed 42), inférence CPU des deux côtés.

Isolation du resize : les DEUX chemins consomment la MÊME image PIL 320x320
(resize bilinéaire une seule fois). On mesure la parité du MODÈLE, pas celle
des pipelines de resize (qui divergeront de toute façon côté Swift — à re-valider
on-device au vrai CH-3).

- Référence : modèle torchvision complet (transform+postprocess d'origine), CPU.
- CoreML : DetModel.mlpackage via coremltools predict, compute_units=CPU_ONLY.
  Si le mlpackage est --nms external, le decode est déjà dedans : on applique
  ici le même postprocess (topk 300, NMS iou 0.55).

Critères (au seuil produit 0.35) :
- boxes appariées (greedy par score décroissant) : IoU >= 0.95
- |Δ score| < 0.02 sur les paires
- top-1 identique : la meilleure box de chaque côté s'apparient à IoU >= 0.95
...
def main() -> None:
    ...
    parser.add_argument("--report", type=Path,
                        default=REPO_ROOT / "ml" / "export" / "parity_report_det.json")
    ...
    nms_mode = mlmodel.user_defined_metadata.get("brickoff.nms", "?")
    ...
    for path in images:
        ...
        tensor = torch.from_numpy(np.asarray(pil)).permute(2, 0, 1).float() / 255.0
        with torch.no_grad():
        ...
        cml_boxes = np.asarray(out["boxes"], dtype=np.float32) * INPUT_SIZE
        cml_scores = np.asarray(out["scores"], dtype=np.float32)
        if external_nms:
            order = np.argsort(-cml_scores)[:300]
            b, s = torch.from_numpy(cml_boxes[order]), torch.from_numpy(cml_scores[order])
            k = nms(b, s, 0.55).numpy()
            cml_boxes, cml_scores = cml_boxes[order][k], cml_scores[order][k]
        keep = cml_scores >= args.score_threshold
        cml_boxes, cml_scores = cml_boxes[keep], cml_scores[keep]

        # --- Appariement greedy par score de référence décroissant ---
        pairs, used = [], set()
        if len(ref_boxes) and len(cml_boxes):
            m = iou_matrix(ref_boxes, cml_boxes)
            for i in np.argsort(-ref_scores):
                cand = [(m[i, j], j) for j in range(len(cml_boxes)) if j not in used]
                if not cand:
                    break
                best_iou, j = max(cand)
                if best_iou <= 0:
                ...
                pairs.append((float(best_iou),
                              float(abs(ref_scores[i] - cml_scores[j])), int(i), int(j)))
        orphans = (len(ref_boxes) - len(pairs)) + (len(cml_boxes) - len(pairs))
        n_orphans += orphans
        ...
        if len(ref_boxes) and len(cml_boxes):
            ...
            j_cml = int(np.argmax(cml_scores))
            top1_ok = bool(iou_matrix(ref_boxes[i_ref:i_ref + 1],
                                      cml_boxes[j_cml:j_cml + 1])[0, 0] >= IOU_MIN)
            n_top1_ok += int(top1_ok)

        per_image.append({
            "image": path.name, "n_ref": len(ref_boxes), "n_coreml": len(cml_boxes),
    ...
    report = {
        "protocol": "CH-3 jalon 3.2 (dry-run)", "date": datetime.now(timezone.utc).isoformat(),
        "weights": str(args.weights), "mlpackage": str(args.mlpackage),
